    ...
    <section class="content-header">
      <h1>
        <?php if ($search_result) : ?>
          ...
          <?= $q; ?>
          براساس
          <?= ($q_category == 'full_name') ? ('نام') : ( ($q_category=='phone')?('شماره تماس'):( ($q_category=='description')?('تفصیل'):('ERROR') ) ); ?>
          به ترتیب
      ...
      <ol class="breadcrumb" style="direction:rtl;">
        ...
        <li class="active">مشترکین دایمی</li>
      </ol>
    </section>
                ...
                <div class="box-body no-padding">
                    <?php if ($search_result) : ?>
                      <table class="table direction table-striped table-bordered table-hover">
                          ...
                          <?php $perm_customer_no = 1; foreach ($search_result as $search_result_item) : ?>
                              <tr>
                                  <td><?= $perm_customer_no; $perm_customer_no++; ?></td>
                                  ...
                                  <td>
                                      <a href="<?= base_url(); ?>perm-customers/<?= $search_result_item->id; ?>/delete" onclick="return confirmation();" class="btn btn-xs btn-block btn-flat bg-red"><i class="fa fa-trash"></i></a>
                                  </td>
                              </tr>
                      ...
                      <!-- show the default TBL -->
                        <table class="table direction table-striped table-bordered table-hover">
                          ...
                          <?php if ($perm_customers) : ?>
                            <?php $perm_customer_no = 1; foreach ($perm_customers as $perm_customer_item) : ?>
                                <tr>
                                    <td><?= $perm_customer_no; $perm_customer_no++; ?></td>
                                    <td><?= $perm_customer_item->full_name; ?></td>
                                    <td><?= $perm_customer_item->phone; ?></td>
                                    <td><?= $perm_customer_item->description; ?></td>
                                    <td>
                                        <a href="#" class="btn btn-xs btn-block btn-flat bg-blue" data-toggle="modal" data-target="#edit_modal_<?= $perm_customer_item->id; ?>"><i class="fa fa-list"></i></a>
                                    </td>
                                    <td>
                                        <a href="<?= base_url(); ?>perm-customers/<?= $perm_customer_item->id; ?>/delete" onclick="return confirmation();" class="btn btn-xs btn-block btn-flat bg-red"><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>

                                <!-- start modal -->
                                <div id="edit_modal_<?= $perm_customer_item->id; ?>" class="modal fade" role="dialog">
                                  <div class="modal-dialog">
                                    <div class="modal-content">
                                      <div class="modal-header">
                                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                                        <h4 class="modal-title">تصحیح <?= $perm_customer_item->full_name; ?></h4>
                                      </div>
                                      <?= form_open(); ?>
                                        <div class="modal-body">
                                          <div class="form-group">
                                            <label>نام</label>
                                            <input type="text" name="full_name" class="form-control" value="<?= $perm_customer_item->full_name; ?>" required="">
                                          </div>
                                          <div class="form-group">
                                            <label>شماره تماس</label>
                                            <input type="text" name="phone" class="form-control" value="<?= $perm_customer_item->phone; ?>">
                                          </div>
                                          <div class="form-group">
                                            <label>تفصیل</label>
                                            <textarea class="form-control" rows="4" name="description"><?= $perm_customer_item->description; ?></textarea>
                                          </div>
                                          <input type="hidden" name="id" value="<?= $perm_customer_item->id; ?>">
                                          <input type="hidden" name="form_code" value="edit">
                                        </div>
                                        <div class="modal-footer">
                                          <button class="btn btn-primary btn-flat bg-blue">ذخیره <i class="fa fa-check"></i></button>
